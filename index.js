...
async function main() {
  const movieStore = new MovieStore('Movie Lounge');
  await movieStore.initialize();

  console.log('\n**********Adding new movies to the store************\n');
  movieStore.addMovie({ title: 'Breathe of Life', totalNumber: 12 });
  movieStore.addMovie({ title: 'Nefarious', totalNumber: 5 });

  console.log('Show available movies\n');
  console.log(movieStore.getMovies());

  console.log('\n**********Renting movies from the store************\n');
  const rented1 = movieStore.rent('Breathe of Life');
  console.log('Successfully rented', rented1);

  const rented2 = movieStore.rent('Nefarious');
  ...
  const returnedMovies = movieStore.returnMovie('Breathe of Life');
